...
export default function MbbsStateSelector() {
  const pathname = usePathname();
  const [open, setOpen] = useState(false);

  const match = pathname.match(/^\/mbbs(?:\/([^/]+))?$/);
  if (!match) return null;

  const currentSlug = match[1] ?? null;
  const currentState = mbbsStates.find((s) => s.slug === currentSlug);
    ...
    >
      ...
      >
        ...
        <span className="truncate min-w-0">
          <span className="hidden sm:inline">{currentState ? `MBBS · ${currentState.name}` : "MBBS · Select State"}</span>
