...
export default function MdMsBranchSelector() {
  ...
  const currentBranch = mdmsBranches.find((s) => s.slug === currentSlug);
    ...
    >
      ...
      >
        ...
        <span className="truncate min-w-0">
          <span className="hidden sm:inline">{currentBranch ? `MD/MS · ${currentBranch.name}` : "MD/MS · Select Branch"}</span>
          <span className="inline sm:hidden">{currentBranch ? currentBranch.name : "Select Branch"}</span>
        </span>
        <svg
      ...
      {open && (
        <div className="absolute right-0 pt-3 w-56 z-50">
          <div className="rounded-md border border-white/15 bg-black/85 backdrop-blur-md shadow-2xl overflow-hidden transform transition-all duration-300">
            <div className="py-1">
              {mdmsBranches.map((branch) => {
                const isActive = branch.slug === currentSlug;
                return (
                  <Link
                    key={branch.slug}
                    href={`/md-ms/${branch.slug}`}
